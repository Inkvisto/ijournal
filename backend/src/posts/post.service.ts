...
export class PostService {
  ...
  async findPosts(params: {
    ...
    if (cursor.id === '' || cursor.id === 'undefined') {
      ...
      return await this.prisma.posts.findMany({
        include: { author: true, comments: true, categories: { select: { category: true } } },
        take
      })
    }
    else {
  ...
  async findOne(id: PostIdArgs) {
    try {
      const { categories, ...post } = await this.prisma.posts.findUnique({ where: { id: id.postId }, include: { author: true, comments: { include: { author: true } }, categories: true }, rejectOnNotFound: true })
